...
In Elm, a list of integers is denoted as `List Int`. List elements
are also enclosed in square brackets, and are separated by commas:
...
characters, thus importing everything that the module exported.
...
pairs by zipping (using the `map2 (,)` function) a list of consecutive
numbers with the list of ten first Fibonacci numbers calculated by
the `fibonacci 10` call.

      > fibonacciWithIndexes 10
      [(0,1),(1,1),(2,2),(3,3),(4,5),(5,8),(6,13),(7,21),(8,34),(9,55)] : List ( Int, Int )
...
The `map2 (,)` function takes two lists, and returns
a list of pairs consisting of consecutive elements from both lists.

      > map2 (,) ["a","b","c","d"] [1..20]
      [("a",1),("b",2),("c",3),("d",4)] : List ( String, number )

Notice, that the first list is shorter than the second one. That is
fine, since `map2 (,)` will only produce a list as long as the shorter of its
arguments.
...
the function application opeator: `f <| a` is equivalent to `f a`,
